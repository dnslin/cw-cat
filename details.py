...
def get_book_data(url):
    ...
    # 获取 tag
    tags = tree.xpath('//p[@class="label-box"]//span[@class="label label-warning J_jubao_tag"]//text()')
    # 清理标签列表
    tags = [tag.strip() for tag in tags if tag.strip() and '举报' not in tag]
    
    # 获取统计数据
    stats = {}
    stats_text = tree.xpath('//p[@class="book-grade"]/text() | //p[@class="book-grade"]/b/text()')
    ...
    # 检查是否为首发
    is_first = bool(tree.xpath('//span[@class="theme-color" and contains(text(), "本站首发")]'))
    if is_first:
